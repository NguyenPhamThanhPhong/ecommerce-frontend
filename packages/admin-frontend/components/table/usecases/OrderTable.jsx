...
export default function OrderTable({ getFilters, orders, onDelete }) {
    const router = useRouter();
    const myData = [];
    if (orders?.length > 0) {
        orders.forEach(order => {
            myData.push(
                createData(order.id, order.code, order?.profile, 
                    order?.createdAt, order?.totalValue, order?.payment?.status));
        })
    }
    function fromDataToRow({ id, code, profile, date, total, status }) {
        return {
            ...
            none: {
                ...
                onView: () => router.push(`/orders/${code}`),
                onEdit: () => router.push(`/orders/${code}`),
            }
        }
    }
    return (
